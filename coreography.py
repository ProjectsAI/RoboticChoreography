...
def compute_matches_original(node):
    accumulator = 0
    counter = 0
...
def compute_matches(node, analyzed_song, dur):
    counter = 0
    round_time = 0
    val_sequences = 0
    for move in node.data:
        round_time = int(round(round_time + move.duration))
        if round_time < dur: #ex: 80
            intensity = analyzed_song[round_time]
        else: break

        val_move = value_BC(move.name, intensity)

        val_sequences+= val_move
        counter+=1
    return val_sequences/counter #media delle valutazioni
...
def search(analyzed_song, dur):
    ...
    matches = [compute_matches(root, analyzed_song, dur)]

    h = [(alpha*compute_time(root)/dur)+(beta*compute_matches(root, analyzed_song, dur))] #prima: alpha= "" /180
    best = root
    found = False
    while not found:
        ...
        for x in reversed(range(1, BRANCHING_FACTOR+1)):
            ...
            matches = [*matches, compute_matches(tree[count-x], analyzed_song, dur)]
            h=[*h, (alpha*compute_time(tree[count-x])/dur)+(beta*compute_matches(tree[count-x], analyzed_song, dur))]
        m = max(h)
        #stop epoch if solutions get too big (time-wise)
        if max(times) > dur+1: #ex: 181
        # if len(tree)>90:
            return False
        index=get_index(h, m)
        best = tree[index]
        #Check solution (According to empirically selected parameters)
        if times[index] > (dur - 0.1)  and times[index] < (dur + 0.1)  and m > 0.94 and m < 0.99: #179.9 , 180.1
            found = True
    return best
...
def expand_node(parent):
    ...
    while check:
        ...
        #PRIMA mand_pos = mandatory_pos[np.random.choice(range(len(mandatory_pos)))]
        mand_pos = final_mandatory_pos[np.random.choice(range(len(final_mandatory_pos)))]
        #select random non mandatory position
        pos = final_optional_moves[np.random.choice(range(len(final_optional_moves)))]
...
final_mandatory_pos= [] #it will be fill later
final_optional_moves = moves
def search_coreography(analyzed_song, dur):
    #Initial Solution Composition
    i=0
    print()
    print("AVAILABLE MANDATORY POSITIONS: ")
    for pos in mandatory_pos:
        print(i, ":", pos.name)
        i+=1

    numbers = input("Choose the mandatory position you want (integer): ")
    print()
    numb= numbers.split()
    for str in numb:
        final_mandatory_pos.append(mandatory_pos[int(str)])
    ...
    numbers = input("Choose the optional position you want to remove (integer, 0 if you don't want any changes): ")
    print()
    numb = numbers.split()
    ...
    for str in numb:
        if int(str)== 0:
            ...
            final_optional_moves.remove(final_optional_moves[int(str) -1 ])



    start = [initial_pos, *final_mandatory_pos, final_goal_pos] #prima: mandatory_pos
